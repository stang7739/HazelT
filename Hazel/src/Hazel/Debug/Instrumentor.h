#ifndef HAZEL_DEBUG_INSTRUMENTOR_H
#define HAZEL_DEBUG_INSTRUMENTOR_H
#include <algorithm>
#include <chrono>
#include <fstream>
#include <iomanip>
	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					HZ_CORE_ERROR("Instrumentor::BeginSession('{}') when session '{}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					HZ_CORE_ERROR("Instrumentor could not open results file '{}'.", filepath);
				}
			}
		}
...
#endif
#endif // HAZEL_DEBUG_INSTRUMENTOR_H
